...
router.get("/grabaciones", async (_, res: Response) => {
  const data = await repo.find({ relations: ["feedbacks", "navegaciones"] });
  res.json(data);
});

router.get("/grabaciones/:id", async (req: Request, res: Response) => {
...
router.post("/grabaciones", async (req: Request, res: Response) => {
  const nuevo = repo.create(req.body);
  const data = await repo.save(nuevo);
  res.status(201).json(data);
});

router.put("/grabaciones/:id", async (req: Request, res: Response) => {
...
router.delete("/grabaciones/:id", async (req: Request, res: Response) => {
